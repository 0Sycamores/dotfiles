...
optimize_mirrors() {
    ...
    if ! command -v reflector &> /dev/null; then
         ...
         run_command "Installing reflector" pacman -S --noconfirm --needed reflector || true
    fi
    ...
    # 同步数据库
    run_command "Syncing pacman databases" pacman -Syy
}
...
check_chezmoi_initialized() {
    print_section_title "Checking Chezmoi Status"
    
    # 默认 Chezmoi 源目录
    if [[ -d "${HOME}/.local/share/chezmoi" ]]; then
        success "Chezmoi is already initialized. Exiting..."
        exit 0
    fi
    info "Chezmoi is not initialized."
}

# 安装并初始化 Chezmoi
install_and_init_chezmoi() {
    print_section_title "Setting up Dotfiles (Chezmoi)"
    
    # 安装 chezmoi
    if ! command -v chezmoi &> /dev/null; then
        run_command "Installing chezmoi" pacman -S --noconfirm --needed chezmoi
    fi
    
    local chezmoi_repo=""
    
    if [[ "${GITHUB_AVAILABLE}" == "true" ]]; then
        chezmoi_repo="0Sycamore"
        info "GitHub is reachable, using username: ${chezmoi_repo}"
    else
        chezmoi_repo="https://arch.sycamore.icu/https://github.com/0Sycamores/dotfiles.git"
        info "GitHub unreachable, using proxy URL: ${chezmoi_repo}"
    fi
    
    # 执行 init
    run_command "Initializing chezmoi" chezmoi init --apply "${chezmoi_repo}"
}
